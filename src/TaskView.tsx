...
function TelegramLinkGate({ user, onLogout }: { user: User; onLogout: () => void }) {
  ...
  // تحقق مباشر من قاعدة البيانات عند فتح الشاشة — لأن بيانات المستخدم
  // المحمّلة قد تأتي من الكاش المحلي (نسخة قديمة قبل الربط)، فتظهر
  // الشاشة لعضو مربوط فعليًا على الأجهزة الجديدة أو التصفح الخفي.
  ...
  useEffect(() => {
    ...
    const verifyFromServer = async () => {
      try {
        const { doc, getDoc } = await import('firebase/firestore');
        const { db } = await import('./firebase');

        // نقرأ المستند مباشرة من Firestore (نتجاوز أي كاش محلي)
        const variants = [
          user.phone,
        ...
        for (const id of variants) {
          ...
          try {
            ...
            if (snap.exists() && snap.data()?.telegramChatId) {
              // مربوط فعليًا — نحدّث الجلسة ونعيد التحميل لتختفي الشاشة
              try {
                const session = JSON.parse(localStorage.getItem('user_session') || '{}');
                session.telegramChatId = snap.data()?.telegramChatId;
                localStorage.setItem('user_session', JSON.stringify(session));
              } catch (e) {}
              if (!cancelled) window.location.reload();
              return;
            }
          } catch (inner) { /* نجرب الصيغة التالية */ }
        }
      } catch (e) {
        console.warn('تعذّر التحقق من حالة ربط تيليجرام:', e);
      }
  ...
  // شاشة انتظار قصيرة أثناء التحقق — تمنع ظهور البوابة خطأً للمربوطين
  if (verifying) {
    return (
      <div
      ...
      >
        <div className="flex flex-col items-center gap-4">
          <div className="w-10 h-10 border-[3px] border-white/25 border-t-white rounded-full animate-spin"></div>
          <p className="text-white/70 text-xs font-bold">جاري التحقق من حالة حسابك...</p>
        </div>
      </div>
    );
  }
  ...
  const handleCheckNow = async () => {
    ...
    setCheckMsg(null);
    try {
      // نقرأ من Firestore مباشرة (لا عبر الكاش) لضمان نتيجة دقيقة
      const { doc, getDoc } = await import('firebase/firestore');
      const { db } = await import('./firebase');
      ...
      for (const id of variants) {
        ...
        try {
          const snap = await getDoc(doc(db, 'users', id));
          if (snap.exists() && snap.data()?.telegramChatId) {
            linked = snap.data()?.telegramChatId;
            break;
          }
        } catch (inner) { /* نجرب الصيغة التالية */ }
      }
