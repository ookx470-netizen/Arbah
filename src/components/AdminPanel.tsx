...
export default function AdminPanel({ adminUser, onLogout, isDarkMode, toggleDarkMode }: AdminPanelProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-800 pb-20" dir="rtl">
          ...
          <div className="flex items-center gap-2">
            {toggleDarkMode && (
              <button
                ...
                className="p-2 bg-white/10 hover:bg-white/20 rounded-lg transition-all cursor-pointer flex items-center justify-center text-white"
                title={isDarkMode ? 'التبديل للوضع المضيء' : 'التبديل للوضع الليلي'}
              >
                {isDarkMode ? <Sun className="w-5 h-5 text-amber-300 fill-amber-300" /> : <Moon className="w-5 h-5 text-slate-100 fill-slate-100" />}
              </button>
            )}
                        ...
                        <tr key={u.id} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="p-3">
                            {isEditing ? (
                              ...
                              <span className="text-slate-600">{u.effectiveDays} يوم</span>
                            )}
                          </td>
                          <td className="p-3">
                ...
                settings.vipPlans.map((plan, idx) => (
                  <div key={plan.id} className="bg-[#070D19] p-3 rounded-xl border border-blue-900/20 flex flex-wrap items-center justify-between gap-2 text-xs font-medium">
                    <div className="flex items-center gap-2 text-right">
                      ...
                      <div>
                        <span className="font-extrabold text-slate-200 block">{plan.name}</span>
            ...
            <div className="p-5 space-y-4 font-semibold text-right text-xs text-slate-700">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setEditVipTierInput(e.target.value)}
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-800 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:bg-white"
                >
          ...
          <div className="bg-white rounded-2xl border border-slate-200 w-full max-w-sm shadow-2xl overflow-hidden animate-scaleIn">
            ...
            {adminNotifications.length > 0 && (
              <div className="p-3 bg-slate-50 border-t border-slate-200 flex justify-between items-center">
                <button
                  onClick={() => markAllNotificationsAsRead(adminUser.phone || adminUser.id || 'admin')}
