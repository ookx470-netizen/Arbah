export interface User {
  ...
  halfEarnings?: boolean; // تخفيض أرباح المهام للنصف — يُفعّل يدويًا من الإدارة لعضو معيّن
  requiredInvitesToUnblock?: number; // عدد الدعوات المطلوبة لفك حظر السحب (يُحدد يدويًا، الافتراضي 2)
  workSuspended?: boolean; // إيقاف العمل (مهام + سحب) — يُفعّل يدويًا، ويُلغى تلقائيًا عند ترقية الباقة
  workSuspendedAtTier?: string; // الباقة وقت الإيقاف — للمقارنة واكتشاف الترقية تلقائيًا
